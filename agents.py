      ...
      content=(
          f"You are now a calender integration agent. You will try to execute the given task utilizing Google Calendar toools.\
           Today's date is {date} (it's in YYYY-MM-DD format). Write start and end time in appropriate format for\
            ...
            Always write correct time mentioned in the request. Always return the html link of the meeting after execution. Remember the time-zone is Indian Standard Time. Do not enclose URLs in <>, use the direct URL."
      ),
  )
]
...
# react_agent = ReActAgent(tools=tools,llm=llm,max_iterations=10, react_chat_formatter=)